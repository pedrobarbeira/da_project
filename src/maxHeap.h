...
public:
    ...
    K removeMax(); // remove and return key with larger value
...
void MaxHeap<K,V>::upHeap(int i) {
    while (i>1 && a[i].value > a[PARENT(i)].value) { // while pos larger than parent, keep swapping to upper position
        swap(i, PARENT(i));
        i = PARENT(i);
    }
}
...
void MaxHeap<K,V>::downHeap(int i) {
    while (LEFT(i) <= size) { // while within heap limits
        int j = LEFT(i);
        if (RIGHT(i)<=size && a[RIGHT(i)].value > a[j].value) j = RIGHT(i); // choose larger child
        if (a[i].value > a[j].value) break;   // node already larger than children, stop
